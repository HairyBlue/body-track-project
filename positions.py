...
class HeartPosition(BodyLandmarkPosition):
    ...
    def get_position(self):
        ...
        x_unity_adjusted = ((x / 100) - 2.5)
        y_unity_adjusted = ((y / 100) - 0.5)
        z_unity_adjusted = (((image_width + z) + 3) / 100) + 12
        print(x_unity_adjusted)
...
class BrainPosition(BodyLandmarkPosition):
    ...
    def get_position(self):
        ...
        normalized_x = int(center_ear[0] * image_width)
        normalized_y = int((1 - center_ear[1]) * image_height - 6)  # Inverting y-coordinate

        ear_position=(normalized_x, normalized_y)
        self.cv2_circle(organ_postion=ear_position)
        return None
